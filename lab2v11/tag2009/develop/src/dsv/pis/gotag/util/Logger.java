...
/**
 * The Logger class is a utility which takes care of log information. The
 * information submitted to the log is treated in three different ways: A
 * timestamp is prepended to each entry. Output may (or may not) be reflected on
 * a file. Output may (or may not) be reflected on a PrintWriter submitted by
 * the application (System.out comes to mind as a natural choice). Writing to a
 * Logger (in theory) never fails, although it may be difficult for the Logger
 * to make a record of that fact.
 */
public class Logger {
	 ...
	 * @param append
	 *            True if the log should be appended to an existing file.
	 */
	...
	/**
	 * Creates a new Logger with output to new file. The logfile name is
	 * composed from the application name and a five-digit random number. The
	 * extension is
	 * 
	 * <pre>
	 * .log
	 ...
	 * 
	 * @param logFileDir
	 *            The directory in which to place the log file.
	 * @param appName
	 *            The application name to be used as prefix in the log file
	 *            name. For example, &quot;MyTest&quot; may result in the log
	 *            file name &quot;MyText95734.log&quot;.
	 */
	 ...
	 *            The path to the logfile.
	 * @param append
	 *            If true, the log is appended to the file.
	 */
	public synchronized void setLogFileName(String logFileName, boolean append) {
		setFile(new File(logFileName), append);
	}

	/**
	 * Composes a name for the logfile. The logfile name is composed from the
	 * application name and a five-digit random number. The extension is
	 * 
	 * <pre>
	 * .log
	 * </pre>
	...
	public synchronized void setLogFile(String logFileDir, String appName) {
		...
		for (int i = 0; i < 10000; i++) {
			File candidate = new File(lfd + File.separator + appName
					+ rnd.nextInt(100000) + ".log");
	 ...
	 * 
	 * @param f
	 *            The File object to use as log file.
	 * @param append
	 *            If true, the log is appended to the file.
	 */
	...
	/**
	 * Makes a log entry. A timestamp and a colon are prepended to the string. A
	 * newline is printed after it. The entry is sent to the logfile (if set)
	 * and the auxilliary PrintWriter (if set). Both streams are flushed (if
	 * set).
	 * 
	 * @param e
	 *            The string to enter into the log.
	 */
	...
	/**
	 * Closes the logfile (if set). The auxilliary PrintWriter is never closed
	 * by the Logger.
	 */
