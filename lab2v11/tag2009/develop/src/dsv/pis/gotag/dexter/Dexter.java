...
public class Dexter implements Serializable {
	/**
	 * The string name of the Bailiff service interface, used when querying the
	 * Jini lookup server.
	 */
	...
	/**
	 * Outputs a diagnostic message on standard output. This will be on the host
	 * of the launching JVM before Dexter moves. Once he has migrated to another
	 ...
	 *             Thrown if the class for the Bailiff service interface could
	 *             not be found.
	 */
	public Dexter(boolean debug, boolean noFace)
		...
		bailiffTemplate = new ServiceTemplate(null,
				new Class[] { java.lang.Class.forName(bfi) }, null);
	}
	 ...
	 * himself a service discovery manager and asks it about Bailiffs. If the
	 * list is long enough, he then selects one randomly and pings it. If the
	 * ping returned without a remote exception, Dexter then tries to migrate to
	 * that Bailiff. If the ping or the migrates fails, Dexter gives up on that
	 * Bailiff and tries another.
	 */
	public void topLevel() throws java.io.IOException {
		...
		for (;;) {
			...
			while (0 < nofItems) {
				...
				// try another. The current (idx) entry in the list of service
				// items
				...
				if (accepted == false) {
					...
					try {
						...
						System.out.println("Jumped to new VM");
